...
sap.ui.define([
	"jquery.sap.global", "../Scene", "./NodeHierarchy", "sap/ui/vk/threejs/thirdparty/totara"
], function(jQuery, SceneBase, NodeHierarchy, totara) {
	"use strict";
	...
	Scene.prototype.nodeRefToPersistentId = function(nodeRefs) {
		if (Array.isArray(nodeRefs)) {
			...
			nodeRefs.forEach(function(nodeRef) {
				ids.push(Totara.object3DToSid(nodeRef));
			});
			return ids;
		} else {
			return Totara.object3DToSid(nodeRefs);
		}
	};
	...
	Scene.prototype.persistentIdToNodeRef = function(pIDs) {
		if (Array.isArray(pIDs)) {
			...
			pIDs.forEach(function(pID) {
				nodeRefs.push(Totara.sidToObject3D(state, pID));
			});
			return nodeRefs;
		} else {
			return Totara.sidToObject3D(this._state, pIDs);
		}
	};
