...
sap.ui.define([
...
], function(jQuery, NumberFormat, BooleanType, DoubleType, DateTimeType, DateTimeOffsetType, DecimalType, Int16Type, Int32Type, Int64Type, SByteType, StringType, TextArrangementStringType, AbapBoolean, CurrencyType, TimeType, GuidType, MetadataAnalyser) {
	"use strict";
	...
	ODataTypes.prototype.getType = function(oProperty, mFormatOptions, mConstraints, mSettings) {
		...
		var oInfo = this._oParent.getBindingInfo("value");
	...
	ODataTypes.prototype.getCurrencyType = function(oProperty) {
		...
		if (oProperty) {
			...
			if (oDecimalConstraints.precision && oDecimalConstraints.scale) {
				var fMaximum = Math.pow(10, (oDecimalConstraints.precision - oDecimalConstraints.scale)) - (1 / Math.pow(10, oDecimalConstraints.scale));

				oCurrencyConstraints = {
					maximum: fMaximum,
					minimum: -1 * fMaximum
				};
			}

			return new CurrencyType(oFormatOptions, oCurrencyConstraints);
		}
