...
sap.ui.define([
	...
	FilterProvider.prototype._createInitialModelForField = function(oJSONData, oFilterFieldMetadata, bUseDefaultValues) {
		var bIsDateTimeType = false, oDefValue, aDefaultFilterValues, oDefaultFilterValue, bHasDefaultFilterValue = false, bIsRangeField = false, sLowValue = null, sHighValue = null, iLength, oItem = null, aItems = [], aRanges = [];
		...
		if (bUseDefaultValues) {
			...
			if (!bHasDefaultFilterValue) {
				...
				if (oFilterFieldMetadata.defaultPropertyValue && oFilterFieldMetadata.isParameter) {
					oDefValue = oFilterFieldMetadata.defaultPropertyValue;

					if (oFilterFieldMetadata.filterType === "numeric") {
						oDefValue = this._getType(oFilterFieldMetadata).parseValue(oDefValue, "string");
					}
							...
							if (oFilterFieldMetadata) {
								...
								if ((oFilterFieldMetadata.filterType === "date") || (oFilterFieldMetadata.filterType === "datetime") || (oFilterFieldMetadata.filterType === "time")) {
									...
									switch (oFilterFieldMetadata.filterType) {
										case "date":
											oFormat = DateFormat.getDateInstance(jQuery.extend({}, this._oDateFormatSettings, {
												UTC: false
											}));
											break;
										case "datetime":
											...
											break;
										case "time":
											oFormat = DateFormat.getTimeInstance(jQuery.extend({}, this._oDateFormatSettings, {
												UTC: false
											}));
											break;
									}
									...
									var bFormatAsDate = ((value1 instanceof Date) || (value2 instanceof Date));
...
// });
							...
							if (oValue.low instanceof Date) {
								...
								if (bIsDateTimeOffsetField) {
									...
									if (aValue) {
										...
										aValue[0] = oFormatter.parse(aValue[0]);
										if (aValue.length === 2) {
											aValue[1] = oFormatter.parse(aValue[1]);
										}
									}
	...
	FilterProvider.parseFilterData = function(oData, oInputJson, mSettings, bReplace) {
		var oResolvedData = {}, mConditionTypeFields = null, sField = null, oValue = null, oNewValue, oJson, i, iLen, oRange, aFilterBarDateFieldNames, aFilterBarTimeFieldNames, aFilterBarTimeIntervalFieldNames, aFilterBarDateTimeMultiValueFieldNames, aDateTimeOffsetValueFields, aViewMetadata, oFieldMetadata, oFilterProvider;
		if (mSettings) {
