sap.ui.define([
	...
	"./KeyboardNavigator"
			 ...
			 ToolbarSpacer, CustomData, Filter, OverflowToolbarLayoutData, KeyboardNavigator) {
	...
	Graph.prototype._fireFailure = function (oType, sMsg) {
		...
		jQuery.sap.log.warning("Graph failure: " + oType + ": " + sParsedMsg);
	};
	...
	Graph.prototype._ensureOnScreen = function (oItem) {
		var oScroller, oSource, iLeft, iRight, iTop, iBottom, iCenterX, iCenterY;
		...
		oSource = (oItem instanceof Line) ? oItem.getSource() : oItem;
		iCenterX = (oSource.getX() + oSource._iWidth / 2) * this._fZoomRatio;
		iCenterY = (oSource.getY() + oSource._iHeight / 2) * this._fZoomRatio;
		iLeft = oScroller.scrollLeft;
		iTop = oScroller.scrollTop;
		iRight = iLeft + this.$scroller.width();
		iBottom = iTop + this.$scroller.height();
		if (iCenterX < iLeft) {
			oScroller.scrollLeft = oSource.getX() * this._fZoomRatio - 10;
		}
		if (iCenterY < iTop) {
			oScroller.scrollTop = oSource.getY() * this._fZoomRatio - 10;
		}
		if (iCenterX > iRight) {
			oScroller.scrollLeft = (oSource.getX() + oSource._iWidth) * this._fZoomRatio + 10 - this.$scroller.width();
		}
		if (iCenterY > iBottom) {
			oScroller.scrollTop = (oSource.getY() + oSource._iHeight) * this._fZoomRatio + 10 - this.$scroller.height();
		}
	};
