...
sap.ui.define([], function() {
	...
	function fnPrepareDeletion(oTemplateContract, mObjectsToDelete){
		...
		var fnHandle = function(iPosition, bDeleted){
			return {
				deleted: bDeleted,
				position: iPosition
			};			
		};
		for (var sPath in mObjectsToDelete){
			var oPromise = mObjectsToDelete[sPath];
			oTemplateContract.oApplicationProxy.prepareDeletion(sPath, oPromise);
			...
			var oDonePromise = oPromise.then(fnHandle.bind(null, iPosition, true), fnHandle.bind(null, iPosition, false));
