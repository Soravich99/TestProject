	...
	function creator(base) {
		...
		var self = {
			...
			mockDemokitService: function () {
				...
				serviceMockServer.attachAfter(sap.ui.core.util.MockServer.HTTPMETHOD.GET, self._postProcessData);
			 ...
			 */
			_postProcessData: function(oEvent) {
				...
				if (oXhr && oFilteredData) {
					...
					if (containsPagination) {
						...
						oFilteredData.results = self._sortData(self._aggregateData(oFilteredData.results || []), parsedUrl.parameters.$orderby);
						oFilteredData.__count = oFilteredData.results.length;
					}
				}
			},
			/**
			 * Aggregate the results returned from the Mockserver. To be called within the _postProcessData-Hook
			...
			/**
			 * Sorts the result-array of the oFilteredData of the _postProcessData-hook. Necessary, as aggregateData requires resorting the entries
			 ...
			 * @returns {Object[]} The sorted results array to replace oFilteredData.results of oEvent of the _postProcessData-hook
			 */
