...
(function() {
	...
	sap.apf.ui.representations.BaseUI5ChartRepresentation.prototype = {
		...
		*/
		setData : function(aDataResponse, metadata) {
			if (this.bIsAlternateView && this.toggleInstance && jQuery.isFunction(this.toggleInstance.setData)) {
				...
				this.oRepresentationFilterHandler.setMetadata(metadata);
				...
				this.oRepresentationFilterHandler.validateFiltersAgainstDataset(this.aDataResponse);
		...
		* @description calls getSelections and sorts the values ascending to text
		* @returns the filter selections of the current representation.
		*/
		getSortedSelections : function(){
			var selections = this.getSelections();
			selections = selections.sort(function(valueA, valueB){
		...
		*/
		destroy : function() {
			...
			if (this.oRepresentationFilterHandler) {
				this.oRepresentationFilterHandler.filterValues = [];
