...
(function() {
	...
	**/
	...
	sap.apf.ui.representations.utils.RepresentationFilterHandler.prototype.validateFiltersAgainstDataset = function(dataResponse) {
		var aValidatedFilterValues = [];
		var aAvailableFiltersInData = dataResponse.map(function(oDataRow) {
			return oDataRow[this.sRequiredFilter];
		}.bind(this));
		aValidatedFilterValues = this.filterValues.filter(function(sFilterValue) {
			return aAvailableFiltersInData.indexOf(sFilterValue) !== -1;
		});
		this.filterValues = aValidatedFilterValues;
	};
	/**
