...
(function() {
	...
	function _selectRowInTreeTable(oEvent) { //mark the selection in tree table and update the filter values
		...
		var requiredFilters = this.oParameters.requiredFilters;
		var aRequiredFilterProperty = requiredFilters && (requiredFilters.length > 0) ? requiredFilters[0] : undefined;
	...
	function _updateFiltersInTreetable(oTreetableInstance, sRequiredFilter, currentSelectedItemIndex) {
		...
		var sDisplayTextForFilter = oTreetableInstance.oTreeTable.getContextByIndex(currentSelectedItemIndex).getProperty(oTreetableInstance.metadata.getPropertyMetadata(sRequiredFilter).text);
		oTreetableInstance.oPaginationDisplayOptionHandler.createDisplayValueLookupForPaginatedFilter(sCurrentModifiedFilter, sDisplayTextForFilter);
	...
	*/
	...
	sap.apf.ui.representations.treeTable.prototype.getSelections = function() {
		...
		this.oRepresentationFilterHandler.getFilterValues().forEach(
				function(selection) {
					sSelectionText = selection;
					if (oTreeTableInstance.parameter.requiredFilterOptions && oTreeTableInstance.parameter.requiredFilterOptions.labelDisplayOption) {
						sSelectionText = oTreeTableInstance.oPaginationDisplayOptionHandler.getDisplayNameForPaginatedFilter(selection, oTreeTableInstance.parameter.requiredFilterOptions, oTreeTableInstance.parameter.requiredFilters[0],
								oTreeTableInstance.oFormatter);
					}
					oSelectionItem.push({
						id : selection,
						text : sSelectionText
	...
	*/
	sap.apf.ui.representations.treeTable.prototype.setFilterValues = function(aValues) {
		...
		var sRequiredFilter = oTreetableInstance.oParameters.requiredFilters[0];
		oTreetableInstance.oRepresentationFilterHandler.clearFilters();
		aValues.forEach(function(value) {
			oTreetableInstance.oPaginationDisplayOptionHandler.createDisplayValueLookupForPaginatedFilter(value[sRequiredFilter], value[oTreetableInstance.metadata.getPropertyMetadata(sRequiredFilter).text]);
			aFilterValues.push(value[sRequiredFilter]);
		});
