sap.ui.define([
	...
	"use strict";
	return BaseController.extend("sap.ui.demo.toolpageapp.controller.BE.ChildrenAllowanceType1", {
				...
				if(pa9701.List[x].child_rep == null){
					var oItem = new sap.m.ColumnListItem({
						cells : [ 	new sap.m.Text({text:pa9701.List[x].objps_for}),
									...
									new sap.m.Text({text:pa9701.List[x].icnum}),
									new sap.m.Text({text:pa9701.List[x].elamt}),
									new sap.m.Input({change:this.onAmtinput}),
									new sap.m.Text({text:""}),
									new sap.m.Text({text:""}),
									new sap.m.Text({text:""})
						]
		...
		onAmtinput: function(oEvent){
			var amt = parseFloat(oEvent.getParameters().value.replace(",",""));
		...
		onSendPressed: function () {
			...
			var selt = this.byId("SelType").getSelectedKey();
			var err = "";
			if(selt == "2"){
			...
			if(err == "X"){
				MessageToast.show("กรณีเบิกส่วนต่างต้องแนบเอกสาร");
			}else{
				var year = new Date().getFullYear();
				var month = (new Date().getMonth() + 1).toString().padStart(2,'0');
				var date = new Date().getDate();
				var url = 'http://webdeqapp.airportthai.co.th:8088/api/aot/v1/tblDocrunning/' + 'BE' + year + month + '/09';
				...
				json.docid = docid;
				json.module = 'BE';
				json.header = "ค่าเงินช่วยเหลือบุตร";
				...
				var seltype = ",seltype:"+this.byId("SelType").getSelectedKey();
				...
				for(var x = 0 ;x < aItems.length;x++){
					...
					{	
						if(i == 8)
						{
							itemData = "seqnr:"+x+",value:"+itemRow[i].getValue().replace(",","");
						}
					}
				...
				var script = spernr+rqust+rqdat+seltype+itemList;
		...
		onSelectType: function (){
			var item = this.byId("SelType").getSelectedKey();
			if(item == "1"){
