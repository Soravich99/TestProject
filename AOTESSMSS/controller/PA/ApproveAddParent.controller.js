sap.ui.define([ 'sap/ui/demo/toolpageapp/controller/BaseController',
	...
	"use strict";
	return BaseController.extend("sap.ui.demo.toolpageapp.controller.PA.ApproveAddParent", {
		...
		onInit : function() {
			...
			for (var i = 0; i < arrayLength; i++) {
			    ...
			    if(field.includes("father") == true){ father = pa0021_arr[i].split(":").pop(); }
			    if(field.includes("mother") == true){ mother = pa0021_arr[i].split(":").pop(); }
	        ...
	        var vfather = this.getView().byId("father");
	        var vmother = this.getView().byId("mother");
			...
			if(father == "true"){father = true;}else{father = false;}
			vfather.setSelected(father);
			if(mother == "true"){mother = true;}else{mother = false;}
			vmother.setSelected(mother);
			...
			oRouter.getRoute("PA/approveAddParent").attachPatternMatched(this._onObjectMatched, this);
...
//			}
		...
		onAccept: function(){
			...
			if(selapp == "ไม่อนุมัติ"){
		        ...
		        $.ajax({
		        	type: "GET",
		        	url: "http://webdeqapp.airportthai.co.th:8088/api/aot/v1/PA0021/"+pernr+"/"+dtype,
		        	async: false,
		        	success: function(data){
		        		pa0021 = data;
		        ...
		        $.ajax({
		        	type: "GET",
		        	url: "http://webdeqapp.airportthai.co.th:8088/api/aot/v1/PA0187/"+pernr+"/"+dtype,
		        	async: false,
		        	success: function(data){
		        		pa0187 = data;
...
//		        		if(pa0187.List[i].pakey.objps == childorder.toString().padStart(2,'0')){
		        			url2 = "http://webdeqapp.airportthai.co.th:8088/api/aot/v1/PA0187/"+pernr+"/"+dtype;
		        			method = "PUT";
		        			break;
//		        		}
		        	}
				...
				ad021.pakey.subty = dtype;
		        ad021.favor = favor;
		        ad021.fanam = fanam;
		        ad021.fgbdt = fgbdt;
		        
		        ad187.pakey.pernr = pernr;
		        ad187.pakey.subty = dtype;
				ad187.jobtl = jobtl;
				ad187.num01 = num01;
				ad187.efaml = efaml;
				ad187.anred = anred;
				if(father == true){ad187.ftxid = ftxid;}
				if(mother == true){ad187.mtxid = ftxid;}
				ad187.telnr = telnr;
				if(aldec == true){
