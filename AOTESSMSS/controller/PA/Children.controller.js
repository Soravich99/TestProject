sap.ui.define([
	...
	"use strict";
	return BaseController.extend("sap.ui.demo.toolpageapp.controller.PA.Children", {
...
//				}
	        ...
	        if(jobtl == "กรุณาเลือกอาชีพ" || vorna.length == 0 || nachn.length == 0 || perid.length == 0 || fgbdt.length == 0 || childstatus.length == 0 || file.length == 0 || deaderr == "X"){
	        	MessageToast.show("Please input all require field");
	        }else if(ch_err == "X"){
	        	MessageToast.show("ลำดับบุตรซ้ำ กรุณาเลือกลำดับอื่น");
	        }else if(ch_err == "ID"){
	        	MessageToast.show("มีบุตรอยู่ในระบบแล้ว");
	        }
	        else{
