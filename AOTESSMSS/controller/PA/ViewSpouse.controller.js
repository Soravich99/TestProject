sap.ui.define([
	...
	"use strict";
	return BaseController.extend("sap.ui.demo.toolpageapp.controller.PA.ViewSpouse", {
		...
		onInit : function() {
	        ...
	        dataPA = $.ajax({
	        	type: "GET",
	        	url: "http://webdeqapp.airportthai.co.th:8088/api/aot/v1/PA0187/"+user+"/1",
	        	async: false,
	        	success: function(data){
	        		pa0187 = data;
			...
			if(pa0187.List.length > 0){
				...
				vstxid.setValue(pa0187.List[0].stxid);
...
//			}
		...
		onSavePressed : function() {
	        ...
	        if(aedtm.length == 0 || favor.length == 0 || fanam.length == 0 || stxid.length == 0 || fgbdt.length == 0 || file.length == 0 || deaderr == "X"){
				...
				json.docid = docid;
				json.module = 'PA';
				json.header = "ขอเปลี่ยนแปลงข้อมูลคู่สมรส";
