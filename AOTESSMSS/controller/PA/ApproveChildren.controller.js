sap.ui.define([
	...
	"use strict";
	return BaseController.extend("sap.ui.demo.toolpageapp.controller.PA.ApproveChildren", {
		...
		onInit: function () {
			...
			for (var i = 0; i < arrayLength; i++) {
				...
				var field = fieldar[0];
				if(field.includes("pernr") == true){var pernr = pa0021_arr[i].split(":").pop(); }
				if(field.includes("aedtm") == true){ aedtm = pa0021_arr[i].split(":").pop(); }
				if(field.includes("jobtl") == true){var jobtl = pa0021_arr[i].split(":").pop(); }
				if(field.includes("anred") == true){var anred = pa0021_arr[i].split(":").pop(); }
				if(field.includes("childorder") == true){var childorder = pa0021_arr[i].split(":").pop(); }
				if(field.includes("childstatus") == true){var childstatus = pa0021_arr[i].split(":").pop(); }
				if(field.includes("nachn") == true){var nachn = pa0021_arr[i].split(":").pop(); }
				if(field.includes("vorna") == true){var vorna = pa0021_arr[i].split(":").pop(); }
				if(field.includes("perid") == true){var perid = pa0021_arr[i].split(":").pop(); }
				if(field.includes("fgbdt") == true){var fgbdt = pa0021_arr[i].split(":").pop(); }
	        ...
	        this.getView().byId("JOBTL").setSelectedKey(jobtl);
	        this.getView().byId("ANRED").setSelectedKey(anred);
	        this.getView().byId("FNMZU").setSelectedKey(fnmzu);
	        this.getView().byId("EFAML").setSelectedKey(efaml);
	        this.getView().byId("orderChildEdit").setSelectedKey(childorder);
	        this.getView().byId("statusChildEdit").setSelectedKey(childstatus);
	        this.getView().byId("TELNR").setValue(telnr);
	        this.getView().byId("NACHN").setValue(nachn);
	        this.getView().byId("VORNA").setValue(vorna);
	        this.getView().byId("PERID").setValue(perid);
			...
			oRouter.getRoute("PA/approveChildren").attachPatternMatched(this._onObjectMatched, this);
...
//			}
		...
		onSavePressed: function () {
			...
			//MessageToast.show("ส่งให้ผู้อนุมัติ");
			this.getRouter().navTo("history");
			var pernr = this.getView().byId("PERNR");
			var ename = this.getView().byId("ENAME");
			var telnr = this.getView().byId("TELNR");
			var ovrna = this.getView().byId("VORNA");
			var nachn = this.getView().byId("NACHN");

			var json = "{";
				  json = json.concat('"PERNR:"',qt,pernr,qt,",");
				  json = json.concat('"ENAME:"',qt,ename,qt,",");
				  json = json.concat('"TELNR:"',qt,telnr,qt,",");
				  json = json.concat('"VORNA:"',qt,ovrna,qt,",");
				  json = json.concat('"NACHN:"',qt,nachn,qt);
				  json = json.concat("}");
				 
			var dataPA = $.ajax({
				type: "PUT",
				url: "http://webdeqapp.airportthai.co.th:8088/api/aot/v1/PA0002/10001",
				async: false,
				data: json,
				success: function(data){
					pa0002 = data;
		...
		onAccept: function (){
	        ...
	        var childstatus = this.getView().byId("statusChildEdit").getSelectedItem().getKey().toString().padStart(2,'0');
			...
			if(selapp == "ไม่อนุมัติ"){
		        ...
		        $.ajax({
		        	type: "GET",
		        	url: "http://webdeqapp.airportthai.co.th:8088/api/aot/v1/PA0021/"+pernr+"/2",
		        	async: false,
		        	success: function(data){
		        		pa0021 = data;
		        ...
		        $.ajax({
		        	type: "GET",
		        	url: "http://webdeqapp.airportthai.co.th:8088/api/aot/v1/PA0187/"+pernr+"/2",
		        	async: false,
		        	success: function(data){
		        		pa0187 = data;
		        ...
		        if(pa0021.List.length > 0){
		        	for(var i=0;i < pa0021.List.length;i++){
		        		if(pa0021.List[i].pakey.objps == childorder.toString().padStart(2,'0')){
		        			url = "http://webdeqapp.airportthai.co.th:8088/api/aot/v1/PA0021/"+pernr+"/2";
		        			method = "PUT";
		        			break;
		        		}
		        ...
		        if(pa0187.List.length > 0){
		        	for(var i=0;i < pa0187.List.length;i++){
		        		if(pa0187.List[i].pakey.objps == childorder.toString().padStart(2,'0')){
		        			url2 = "http://webdeqapp.airportthai.co.th:8088/api/aot/v1/PA0187/"+pernr+"/2";
		        			method = "PUT";
		        			break;
		        		}
		        	}
				...
				fgbdt = [parseInt(fgbdts[2]),fgbdts[1],fgbdts[0].toString().padStart(2,'0')].join("-");
		        ...
		        ad021.fanam = nachn;
		        ad021.fgbdt = fgbdt;
		        ad021.zzcstat = childstatus;
		        ad021.pakey.objps = childorder.toString().padStart(2,'0');
				...
				ad187.pakey.objps = childorder.toString().padStart(2,'0');
