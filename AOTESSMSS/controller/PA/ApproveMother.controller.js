sap.ui.define([ 'sap/ui/demo/toolpageapp/controller/BaseController',
	...
	"use strict";
	return BaseController.extend("sap.ui.demo.toolpageapp.controller.PA.ApproveMother", {
		...
		onInit : function() {
			...
			for (var i = 0; i < arrayLength; i++) {
			    ...
			    if(field.includes("mtxid") == true){ mtxid = pa0021_arr[i].split(":").pop(); }
			...
			oRouter.getRoute("PA/approveMother").attachPatternMatched(this._onObjectMatched, this);
...
//			}
		...
		onAccept : function () {
			...
			if(selapp == "ไม่อนุมัติ"){
		        ...
		        $.ajax({
		        	type: "GET",
		        	url: "http://webdeqapp.airportthai.co.th:8088/api/aot/v1/PA0021/"+pernr+"/12",
		        	async: false,
		        	success: function(data){
		        		pa0021 = data;
		        ...
		        $.ajax({
		        	type: "GET",
		        	url: "http://webdeqapp.airportthai.co.th:8088/api/aot/v1/PA0187/"+pernr+"/12",
		        	async: false,
		        	success: function(data){
		        		pa0187 = data;
