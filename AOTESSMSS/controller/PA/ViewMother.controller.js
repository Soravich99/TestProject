sap.ui.define([ 'sap/ui/demo/toolpageapp/controller/BaseController',
	...
	"use strict";
	return BaseController.extend("sap.ui.demo.toolpageapp.controller.PA.ViewMother", {
		...
		onInit : function() {
	        ...
	        var vmtxid = this.getView().byId("MTXID");
			...
			if(pa0021.List.length > 0){
				...
				var aedtm = "";
				aedtm = new Date(pa0021.List[0].pakey.begda);
			...
			if(pa0187.List.length > 0){
				...
				vmtxid.setValue(pa0187.List[0].mtxid);
		...
		onSavePressed : function() {
	        ...
	        var mtxid = this.getView().byId("MTXID").getValue();
	        ...
	        if(aedtm.length == 0 || favor.length == 0 || fanam.length == 0 || mtxid.length == 0 || fgbdt.length == 0 || file.length == 0 || deaderr == "X"){
				...
				var url = 'http://webdeqapp.airportthai.co.th:8088/api/aot/v1/tblDocrunning/' + 'PA' + year + month + '/05';
				...
				json.docid = docid;
				json.module = 'PA';
				json.header = "ขอเปลี่ยนแปลงข้อมูลมารดา";
